...
## Statistics
- **generated**: {{total}}
- **extracted**: {{extr\_all}} ({{extr\_all\_perc|round(2)}} %) <br>
  contains:
  - **one-to-one**: {{extr\_ok}} ({{extr\_ok\_perc|round(2)}} %)
  - **overlapped**: {{extr\_ovlpd}} ({{extr\_ovlpd\_perc|round(2)}} %)
  - **false pos**: {{extr\_fpos}} ({{extr\_fpos\_perc|round(2)}} %)

## Amplitude histograms
{{ hist\_plot }}

## Amplitude detection errors
{{ amp\_errs\_plot }}

## Time detection errors
{{ times\_errs\_plot }}
"""
