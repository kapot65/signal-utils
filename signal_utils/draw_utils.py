...
class graphics(object):
    ...
    def __call__(self, *args, **kwargs):
        """Import graphics and call function."""
        import_graphics()
        self.f(*args, **kwargs)
...
def plot_event(data, num, event, ax, threshold=700, bin_sec=3.2e-07):
    ...
    ax.set_title("ampl: %s, size: %s (%s s)" % (data[extremum], lenght,
                                                lenght * bin_sec))

    ax.plot(data, label="event")
    ax.plot(
        (0, len(data)), (threshold, threshold),
        "--",
        color="green",
        label="first greater")
    ax.plot(
        ...
        color="orange",
        label="first greater")
    ax.plot((extremum, extremum), y_range, "--", color="red", label="extremum")
    ax.plot(
...
def draw_metrics(metrics: dict):
    ...
    def get_dbl_amps_dists(idxs):
        ...
        for idx in idxs:
            ...
            amps.append(np.log(doubles_amps[1:], doubles_amps[:-1]))
    ...
    ax[2][0].hist2d(
        dbl_dists_real.astype(np.float64), dbl_amps_real.astype(np.float64))
    ax[2][1].set_title("detected doubles")
    ax[2][1].hist2d(
        dbl_dists_det.astype(np.float64), dbl_amps_det.astype(np.float64))
    ...
    range_ = (min(metrics["amps_real"].min(), metrics["amps_extracted"].min()),
              max(metrics["amps_real"].max(), metrics["amps_extracted"].max()))

    ax[1][0].set_title("real vs generated amps hists")
    ax[1][0].hist(metrics["amps_real"], 80,
                  fc=(1, 0, 0, 0.5), label="real", range=range_)
    ax[1][0].hist(
        metrics["amps_extracted"],
        80,
        fc=(0, 0, 1, 0.5),
        label="extracted",
        range=range_)
    ...
    range_ = (min(metrics["pos_real"].min(), metrics["pos_extracted"].min()),
              max(metrics["pos_real"].max(), metrics["pos_extracted"].max()))

    ax[3][0].set_title("real vs generated pos hists")
    ax[3][0].hist(metrics["pos_real"], 80,
                  fc=(1, 0, 0, 0.5), label="real", range=range_)
    ax[3][0].hist(
        metrics["pos_extracted"],
        80,
        fc=(0, 0, 1, 0.5),
        label="extracted",
        range=range_)
...
def draw_real_frame(meta, metrics, idx, real=True, frame_l=15, frame_r=25):
    """Отрисовка кадра реального события.

    На графике также указываются амплитуды и времена реальных и выделенных
    событий.
    ...
    ax.plot(
        poses_real / (1e+9 * freq),
        metrics['amps_real'][l_index_r:r_index_r],
        'ro',
        label='real events')
    ax.plot(
        poses_extr / (1e+9 * freq),
        metrics['amps_extracted'][l_index_e:r_index_e],
        'bo',
